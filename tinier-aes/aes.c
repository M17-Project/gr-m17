...
// Secure fallback for systems without explicit_bzero
...
void aes_ofb_keystream_output (uint8_t * iv, uint8_t * key, uint8_t * output, int type, int nblocks)
{
  ...
  // SECURITY FIX: Use secure memory clearing for AES round keys
  explicit_bzero(ctx.RoundKey, 240*sizeof(uint8_t));
  KeyExpansion(ctx.RoundKey, key);
